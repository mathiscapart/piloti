...
- **Au seuil du changement de mot de passe, la session qui insiste est fermée et le titulaire prévenu** (choix produit validé le 2026-09-22). Cinq mots de passe actuels faux d'affilée sont un signal fort de session volée : refuser ne suffit pas, l'attaquant garderait ses droits. La session est supprimée dans la même transaction que son audit (`USER_SESSION_REVOKED`), puis redirection vers `/login?locked=1`. Le titulaire reçoit une `SECURITY_ALERT` forcée, au plus une par heure et par compte. Les autres sessions du compte restent ouvertes : ne fermer que celle qui insiste épargne le titulaire connecté ailleurs. Une autre session qui essaie pendant le blocage est refusée sans être fermée ; si elle atteint le seuil à son tour, dans une fenêtre suivante, elle est fermée elle aussi.
