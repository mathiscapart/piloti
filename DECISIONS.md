...
**Choix** :
- Le verrou passe dans **`src/proxy.ts`**, qui voit chaque requête, fetchs RSC compris. Il lit `birthDate` depuis la session better-auth (aucun `cookieCache` configuré, donc valeur fraîche, sans requête DB supplémentaire). Celui du layout est conservé en défense en profondeur, et l'en-tête `x-pathname` reste transmis : le layout s'en sert toujours pour épurer le chrome sur l'écran de complétion.
- `completeBirthDate` refuse explicitement quand la date est déjà présente, et `/completer-profil` redirige vers `/dashboard` si le profil est complet — les deux conditions sont complémentaires, aucune boucle possible.
...
- Bornes de plausibilité `MIN_PLAUSIBLE_AGE = 5` (marge sous les Farfadets, 6 ans) et `MAX_PLAUSIBLE_AGE = 110`, appliquées partout puisque les trois chemins d'écriture partagent `birthDateSchema`. Les deux contrôles deviennent des `refine`, évalués à la validation : le `.max(new Date())` précédent figeait « maintenant » au chargement du module et dérivait sur un serveur au long cours.
- `prisma/seed.ts` renseigne désormais une date pour les quatre comptes de démo. Ce n'est pas cosmétique : sans elle, le verrou désormais étanche renvoie **tous** les comptes seedés sur l'écran de complétion, et plus aucun écran de l'app n'est atteignable en développement.

**Conséquences** : plus aucun chemin self-service ne permet de modifier une date de naissance existante, et toute correction laisse une trace nominative. En contrepartie, une faute de frappe à l'inscription **exige l'intervention d'un administrateur** — c'est le prix assumé de la protection des mineurs, et la raison pour laquelle l'éditeur admin est livré dans le même lot plutôt que renvoyé à plus tard. Le contournement a été rejoué avant/après sur la même base : `RSC=200` sans le verrou proxy, `RSC=307` avec.
