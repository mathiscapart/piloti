...
- Contrôles ad hoc « ADMIN » : ouverts au RG quand ils relèvent de la gestion du groupe (modifier ou archiver le lieu d'un autre, `canManagePlace` ; épingler un message et clore le sondage d'un autre, nouvelle action `channel.moderate` pour CHEF et RG ; supprimer le message d'un autre dans l'interface). Modifier ou supprimer le message d'un autre exige en plus l'accès au salon : le RG n'a pas le passe-droit de salon de l'ADMIN. Restent à l'ADMIN ceux qui relèvent de la technique ou de la protection des mineurs : accès à tous les salons, écriture dans un salon archivé, passe-droit de messagerie privée (SAFE-01).
