...
- **Pédagogique** (précisé par le responsable le 2026-09-24) : le suivi des jeunes reste aux chefs de branche. Le RG n'a ni `pedago.manage` (proposer ou confirmer une étape, attribuer un badge, objectifs) ni `pedago.referential` (référentiel d'étapes, catalogue de badges). Il garde la lecture (`pedago.view`) et l'annulation d'une étape confirmée (`pedago.validation.cancel`, #100).
- **Notes de suivi sensibles (US-S07)** : #98 est maintenu, le RG ne les lit ni ne les écrit. `canReadPedagoNotes` évalue le droit sans le rôle RG, sans quoi un RG aussi CHEF, que `inUnitScope` ne borne pas, les lirait dans toutes les branches. Elle décide aussi de l'écriture (`addNote`, `deleteNote`). Un RG aussi CHEF les lit comme un chef, sur sa branche.
